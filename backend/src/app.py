...
BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))  # Points to /abc/
FRONTEND_SRC_DIR = os.path.join(BASE_DIR, 'frontend', 'src')
STATIC_ASSETS_DIR = os.path.join(FRONTEND_SRC_DIR, 'assets')

# Serve static assets
app.mount("/static", StaticFiles(directory=STATIC_ASSETS_DIR), name="static")

# If you still want to serve built React files, adjust this:
BUILD_DIR = os.path.join(BASE_DIR, 'frontend', 'build')
if os.path.exists(BUILD_DIR):
    app.mount("/", StaticFiles(directory=BUILD_DIR, html=True), name="root")

# Serve React app for root and all other routes
@app.get("/", response_class=FileResponse)
@app.get("/{path:path}", response_class=FileResponse)
async def serve_react_app(path: str = ""):
    return os.path.join(REACT_BUILD_DIR, 'index.html')
